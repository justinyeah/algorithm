...
template <typename T>
int generate_random(T range_l, T range_r){
    std::random_device gt;
    std::mt19937 random_gt(gt());
    std::uniform_int_distribution<int> distribution(range_l, range_l);
    int rand_number = distribution(random_gt);
    return rand_number;
}

template <typename T>
void select_sort(std::vector<T>& nums) {
...
template <typename T>
void insert_sort(std::vector<T>& nums) {
    if (nums.size() < 2) return;
    for (int i = 0; i < nums.size(); i++) {
...
template <typename T>
void merge_sort(std::vector<T>& nums, int i, int j) {
    if (i == j) return;
    int mid = (i + j) / 2;
...
template <typename T>
std::vector<T> partition(std::vector<T>& nums, int i, int j) {
    int l = i - 1, r = j + 1;
    T partition_num = nums[j];
    int index = i;
    while (index < r) {
...
template <typename T>
void quick_sort(std::vector<T>& nums, int i, int j) {
    if (i >= j) return;
...
template <typename T>
void heap_insert(std::vector<T>& nums, int i) {
...
template <typename T>
void heapify(std::vector<T>& nums, int i, int size) {
    int lc = 2 * i + 1;
    while (lc < size) {
